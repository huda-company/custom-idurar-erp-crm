...
methods.create = async (req, res) => {
  try {
    ...
    const { total, discount, credit } = result.invoice
    console.log('🚀 ~ file: paymentInvoiceController.js ~ line 63 ~ methods.create= ~ total', total)
    ...
    const [updatedResult] = await Promise.all([updatePath, invoiceUpdate])
    res.status(200).json({
      success: true,
